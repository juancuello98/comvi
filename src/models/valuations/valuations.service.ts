...
export class ValuationsService {
  ...
  async create(createValuationDto: CreateValuationDto):Promise<ResponseDTO> {
...
try {
  const hasTrip = await this.tripModel.findById(createValuationDto.tripId);
    ...
    if(!hasTrip) 
    {
      ...
      return this.responseHelper.makeResponse(false, `Not found trip ${createValuationDto.tripId} for user ${createValuationDto.email}`,null,HttpStatus.NOT_FOUND);
    }
    ...
    return this.responseHelper.makeResponse(false, `A new valuation was added to the trip ${createValuationDto.tripId} for user ${createValuationDto.email}`,updatedValuation,HttpStatus.CREATED);

} catch (error) {
  this.logger.log('Error in create: ',error);

  return this.responseHelper.makeResponse(true,"The valuation was not created",null,HttpStatus.INTERNAL_SERVER_ERROR);
}
   ...
   async findAll(email: string):Promise<ResponseDTO> {
    ...
    try {
      ...
      if(items.length == 0) return this.responseHelper.makeResponse(false,message,null,HttpStatus.NOT_FOUND);
    ...
    try {
      ...
      if(!valuation) return this.responseHelper.makeResponse(false,message,null,HttpStatus.NOT_FOUND);

      message = 'Valuation Successfully founded ';

      return this.responseHelper.makeResponse(false,message,valuation,HttpStatus.OK);
  ...
  try {
    const hasValuation = await this.valuationModel.findByIdAndUpdate(updateValuationDto.id,updateValuationDto); //El DTO debe traer los valores no cambiados
      
      if(!hasValuation) 
      {
        this.logger.log(`Not found valuation ${updateValuationDto.id} for user ${updateValuationDto.email}`)
        return this.responseHelper.makeResponse(false, `Not found valuation ${updateValuationDto.id} for user ${updateValuationDto.email}`,null,HttpStatus.NOT_FOUND);
      }
      this.logger.log(`The valuation of the trip ${updateValuationDto.tripId} for user ${updateValuationDto.email} was updated`);
      return this.responseHelper.makeResponse(false, `The valuation of the trip ${updateValuationDto.tripId} for user ${updateValuationDto.email} was updated`,hasValuation,HttpStatus.CREATED);
  ...
  try {
    const hasValuation = await this.valuationModel.findByIdAndDelete(id); //El DTO debe traer los valores no cambiados
      ...
      if(!hasValuation) 
      {
        ...
        return this.responseHelper.makeResponse(false, `Not found valuation ${id} for user`,null,HttpStatus.NOT_FOUND);
      }
      this.logger.log(`The valuation of the trip ${hasValuation.tripId} for user ${hasValuation.email} was deleted`);
      return this.responseHelper.makeResponse(false, `The valuation of the trip ${hasValuation.tripId} for user ${hasValuation.email} was deleted`,hasValuation,HttpStatus.CREATED);
