...
export class FuelsStationMongodbRepository implements IFuelsStationRepository {
    ...
    async findByGeometry(geometry: { type: string, coordinates: number[] }): Promise<FuelsStation[]>{

        return await this.FuelsStationStationModel.find(
            {
            geojson: {
                $near: {
                    $geometry: geometry,
    ...
    async addCompanies(companies: FuelsStation[]): Promise<FuelsStation[]> {
        const bulkOps = companies.map(FuelsStation => ({
            updateOne: {
                filter: { stationId: FuelsStation.stationId },
